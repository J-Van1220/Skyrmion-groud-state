...
#transient parameters, distrubute resources, compute layer energy
    def compute_energy(self):
        ...
        threadsperblock = (16, 16)#one block contain 16*16 thread
...
#gradient descent
    def gradient_descent(self, rho=0.95, epsilon=1e-6, max_iter=1000, tol=1e-6,
        ...
        # 生成动画
        if save_frames and animate:
            try:
                import imageio.v2 as imageio
                import glob, os
                images = []
                pattern = os.path.join(frame_dir, f"layer1_iter*.png")
                file_list = sorted(glob.glob(pattern))
                for fname in file_list:
                    images.append(imageio.imread(fname))
                if images:
                    imageio.mimsave(animation_path, images, duration=0.2)
                    print(f"动画已保存: {animation_path}")
            except ImportError:
                print("未安装 imageio，无法生成动画。请先安装: pip install imageio")
        return final_energy, energy_history
    
#initalize spin
    def initialize_spins(self, method="skyrmion", r=10):
        if method == "skyrmion":
            center = self.layers[0].N // 2
            for i in range(self.layers[0].N):
                for j in range(self.layers[0].N):
                    if (i - center)**2 + (j - center)**2 < r**2:
                        self.layers[0].phi[i, j] = 0.0
                        self.layers[0].theta[i, j] = 0.0  # Center up
                        if len(self.layers) > 1:
                            self.layers[1].phi[i, j] = np.pi
                            self.layers[1].theta[i, j] = 0.0  # Center down
                    else:
                        self.layers[0].phi[i, j] = np.pi
                        self.layers[0].theta[i, j] = 0.0  # Background down
                        if len(self.layers) > 1:
                            self.layers[1].phi[i, j] = 0.0
                            self.layers[1].theta[i, j] = 0.0  # Background up
        elif method == "random":
            for layer in self.layers:
                layer.phi = np.random.uniform(0, 2 * np.pi, (layer.N, layer.N))
                layer.theta = np.random.uniform(0, np.pi, (layer.N, layer.N))
        else:
...
def simulate_system(num=1):
    if num==1:
        ...
        print(f"Simulating Bilayer System with Manuscript Parameters:")
    N = 64
    r = 10  
    ...
    if num == 1:
        ...
        system.initialize_spins("random", r)#setting initial condition
    elif num == 2:
        Bx1, By1, Bz1 = get_magnetic_field(N, 0.0)
        Bx2, By2, Bz2 = get_magnetic_field(N, 0.0)
        layer1 = Layer(N, Jx=1.0, Jy=1.0, Dx=0.03,Dy=0.03, K=-0.04, A=0, Bx=Bx1, By=By1, Bz=Bz1)#setting paramters according to report
        layer2 = Layer(N, Jx=1.0, Jy=1.0,Dx=0.03, Dy=0.03, K=-0.04, A=0, Bx=Bx2, By=By2, Bz=Bz2)#setting paramters according to report
        system = MultilayerSystem([layer1, layer2], J_AFM=0)#setting paramters according to report
        system.initialize_spins("skyrmion", r)#setting initial condition
    
    for i in range(len(system.layers)):
        system.visualize(i, f"Initial Layer {i+1}")
        
    final_energy, energy_history = system.gradient_descent(max_iter=1000, save_frames=True, frame_every=50,
