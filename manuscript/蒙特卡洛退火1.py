...
def compute_local_energy(i, j, phi, theta, N, Jx, Jy, Dx, Dy, K, B_field):
...
@cuda.jit
def monte_carlo_step(phi, theta, N, T, rng_states, parity, Jx, Jy, Dx, Dy, K, B_field):
    i, j = cuda.grid(2)
    if i < N and j < N and (i + j) % 2 == parity:  # Checkerboard update (odd/even parity)
        ...
        old_energy = compute_local_energy(i, j, phi, theta, N, Jx, Jy, Dx, Dy, K, B_field)
        ...
        theta[i, j] = theta_new
        # Compute new energy
        new_energy = compute_local_energy(i, j, phi, theta, N, Jx, Jy, Dx, Dy, K, B_field)
...
class SpinSystem:
    def __init__(self, N, Jx=1.0, Jy=1.0, Dx=1.5, Dy=1.5, K=0.2, B=0.5, r0=0.1):
        ...
        self.K = K                     # Perpendicular magnetic anisotropy
        ...
        for step in range(steps):
            ...
            for _ in range(sweeps):
                ...
                monte_carlo_step[blocks, threads](
                    ...
                    0, self.Jx, self.Jy, self.Dx, self.Dy, self.K, self.B_field)
                monte_carlo_step[blocks, threads](
                    self.phi, self.theta, self.N, T, self.rng_states,
                    1, self.Jx, self.Jy, self.Dx, self.Dy, self.K, self.B_field)
    ...
    def energy(self):
        ...
        @cuda.jit
        def energy_kernel(sx, sy, sz, N, Jx, Jy, Dx, Dy, K, B_field, energy):
            i, j = cuda.grid(2)
            if i < N and j < N:
                right = (i + 1) % N, j
                up = i, (j + 1) % N
                ...
                other = K*(sz[i,j]**2) - B_field[i,j]*sz[i,j]
        ...
        energy_kernel[blocks, threads](
            ...
            self.Jx, self.Jy, self.Dx, self.Dy, self.K, self.B_field, self.d_energy)
...
if __name__ == "__main__":
    ...
    system = SpinSystem(
        N=65,          # Grid size
        Jx=6.0e-3,     # Ferromagnetic exchange
        Jy=6.0e-3,
        Dx=-1.4e-3,    # DMI (increased for chirality)
        Dy=1.4e-3,
        K=5.0e-5,      # Perpendicular anisotropy
        B=4.0,         # Magnetic field magnitude
        r0=0.1         # Downward field radius
    )
    
    # Initialize with a single Skyrmion
    system.skyrmion_initialize(center_x=32, center_y=32, radius=8.0, chirality=1)
    ...
    system.simulate(T_start=10.0, T_end=0.01, steps=10, sweeps=10000)
